...
class RoomScreen(
    ...
    override fun onGetTemplate(): Template {
        ...
        itemListBuilder.addItem(
            Row.Builder()
                .setTitle(carContext.baseContext.getString(de.christian2003.smarthome.data.R.string.car_reload))
                .addText(carContext.baseContext.getString(de.christian2003.smarthome.data.R.string.car_reload_info))
                .setOnClickListener {
    ...
    private fun buildRow(text: ShInfoText): Row {
        ...
        builder.setTitle(text.label.ifEmpty { "No title provided" })
        if (text.text != null) {
    ...
    private fun buildRow(device: ShGenericDevice): Row {
        return when (device) {
            is ShGenericDevice -> buildRowGeneric(device)
    ...
    private fun buildRowGeneric(device: ShGenericDevice): Row {
        val builder = Row.Builder()
        builder.setTitle(device.name.ifEmpty { "No name provided" })
        if (device.specifier != null && device.specifier!!.isNotEmpty()) {
            builder.addText(device.specifier!!)
        }
        else if (device.imageUri != null && device.imageUri!!.isNotEmpty()) {
            val name = device.imageUri!!.substringAfterLast("/").substringBeforeLast(".")
            if (name.isNotEmpty()) {
                builder.addText(name.replaceFirstChar { it.uppercaseChar() })
            }
        }
        return builder.build()
    }
